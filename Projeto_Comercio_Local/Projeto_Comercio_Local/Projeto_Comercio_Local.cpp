...
void adicionarProduto()
{
	...
	if (nProdAdicionar == 1)
	{
		cout << GREEN << endl << "Produto adicionado.\n" << RESET;
	}
	// Se adicionar mais que 1
	if (nProdAdicionar > 1)
	{
		cout << GREEN << endl << "Produtos adicionados.\n" << RESET;
	}
	...
	for (int n = 0; n < nProdEliminar; n++)
	{
		...
		for (int i = 0; i < totalProdutos; i++)
		{
			if (produto[i][0] == id)
			{
				...
				cout << RED <<  endl << "Produto eliminado. \n" << RESET;
				encontrado = true;
				break;
			}
		}
...
void modificarPreco() {
	...
	{
		produtosDisponiveis();
		cout << endl << "Insira o ID do produto a qual deseja modificar o preco ou 'SAIR' para retornar ao Menu Principal: ";
		cin >> id;
		id = toLower(id);
		...
		for (int i = 0; i < totalProdutos; i++)
		{
			...
			{
				encontrado = true;
				cout << endl << "Preco atual: " << produto[i][3] << endl;
				cout << endl << "Insira o novo preco do produto: ";
...
void menu(string** carrinho) {
	...
	do {
		...
		case 6:
			...
			break;
		case 7:
			cout << RED << endl << "A desligar o progama...\n" << RESET;
			break;
		default:
			// Opcao invalida
			cout << endl << "Opcao invalida. Tente novamente.\n";
